	...
	<view class="rf-category">
		...
		<view
			...
			<!-- 		<view
						...
						<view class="box">
							<!-- <view class="second-category-text" @tap="navTo(`/pages/product/list?cate_id=${item.id}`)">
								{{ item.title }}
							...
							<view class="list" v-if="categoryProductList.length>0">
								<view class="box"
									...
									:key="item.id"
									@tap.stop="navTo(`/pages/product/list?cate_id=${item.id}`)"
								>
									<block >
										<image :src="item.pic || errorImage"></image>
										<view class="text">{{ item.name }}</view>
									</block>
									<text class="category-tag in1line" >
										{{ item.name }}
									</text>
								</view>
							</view>
						...
						<!-- <view class="no-data" v-if="item.child.length === 0">
							<i class="iconfont icon404"></i>
							暂无分类
						</view> -->
					</view>
    ...
    ...mapMutations(['setCartNum']),
      ...
      this.categoryInvertedIndex = index;
		...
		async initData() {
			...
			this.hotSearchDefault = this.hot_search_default || '请输入关键字';
			this.categoryList = [];
			this.categoryProductList = [];
			...
			params.pageSize = 10;
			 this.$http
				.get(`${productList}`, {
					...
					this.categoryProductList = [...this.categoryProductList, ...data];
