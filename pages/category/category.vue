					...
					<view
						...
						>
							...
							<view class="list" >
								<view
									class="box"
									v-for="(sItem, fIndex) in categoryProductList"
									:key="sItem.id"
									@tap.stop="navTo(`/pages/product/list?cate_id=${sItem.id}`)"
								>
									<block v-if="styleCateType === 'one_two_three_cover'">
										<image :src="sItem.icon || errorImage"></image>
								...
								<view v-if="categoryProductList.length===0" class="no-data">
    ...
    ...mapMutations(['setCartNum']),
      ...
      this.categoryInvertedIndex = index;
		...
		async getProductCate(type) {
			console.log(this.styleCateType,'this.styleCateType')
			await this.$http
				.get(`${productCate}`)
				.then(async r => {
					if (type === 'refresh') {
                ...
                this.showCategoryIndex = index;
						...
						if (this.styleCateType === 'one_two_product' && r.data[0].children.length > 0) {
							this.currentCateId = r.data[0].children[0].id;
						} else {
					...
					if (r.data.length > 0) {
						this.currentSecondCategoryList = r.data[0].children;
					}
					this.page = 1;
					
					this.categoryProductList = this.categoryList[0].children;
					console.log(this.categoryList[0].children,998)
		...
		showCategory(index) {
			this.showCategoryIndex = index;
			this.categoryProductList = this.categoryList[index].children;
		},
