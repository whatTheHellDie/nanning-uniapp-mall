		...
		<view :class="isOpenIndexCate && categoryList.length > 0 ? 'main-wrapper' : ''">
			<block v-if="currentCate === 0">
				...
				<swiper
					...
					indicator-color="#666"
					:indicator-dots="brandList.length > 10"
					:style="{height: brandList.length <= 5 ? '200rpx' : '400rpx'}"
					...
					>
					<!-- v-for="(sItem, sIndex) in fItem" :key="sIndex" -->
						<view
							class="category"
							v-for="(item, fIndex) in brandList"
							:key="fIndex"
							@tap.stop="navToCategory(item.id)"
							 >
							<view class="img">
								<image :src="item.logo || errorImage" mode="aspectFill"></image>
							</view>
							<view class="text in1line">{{ item.name }}</view>
						</view>
					</swiper-item>
				</swiper>
				<!--新闻通知-->
...
export default {
	...
	methods: {
		...
		initIndexData(data) {
			...
			this.announceList = data.advertiseList || [];
			this.productCateList = data.advertiseList|| [];
			...
			this.hotSearchDefault = data.hot_search_default || '请输入关键字';
			uni.setStorage({
				key: 'hotSearchDefault',
				data: data.hot_search_default
			});
			this.hotProductList = data.hotProductList|| [];
			console.log(this.hotProductList,'1122')
