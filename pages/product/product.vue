<template>
    ...
    <!--回到顶部-->
		...
		<!-- 404页面 -->
		<view v-if="!productDetail.product && !loading">
			<rf-no-data :custom="true">
				<view class="no-data-title">
					{{ errorInfo || '暂无数据' }}
				</view>
				<view
					@tap="getProductDetail"
					slot="refresh"
					class="spec-color"
					>重新加载</view
				>
		...
		async getProductDetail() {
			await this.$http
				...
				.then(async r => {
					...
					uni.setNavigationBarTitle({ title: r.data.product.name });
					await this.$mHelper.handleWxH5Share(this.appName, r.data.product.name, this.currentUrl, r.data.product.pic);
				})
				.catch(err => {
					this.loading = false;
					this.errorInfo = err;
